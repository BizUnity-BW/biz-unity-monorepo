...
export function createCustomer(orgId: string, data: { name: string; email?: string; phone?: string; address?: string }) {
  return prisma.customer.create({ data: { ...data, orgId } });
}

export function updateCustomer(id: string, orgId: string, data: Partial<{ name: string; email: string; phone: string; address: string }>) {
