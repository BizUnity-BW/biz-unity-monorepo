...
export async function get(req: Request, res: Response): Promise<void> {
  const item = await service.getInvoice(req.params.id, (req as AuthenticatedRequest).org.id);
...
export async function updateStatus(req: Request, res: Response): Promise<void> {
  ...
  if (!parsed.success) { fail(res, 'Validation failed', 422, parsed.error.flatten()); return; }
  await service.updateInvoiceStatus(req.params.id, (req as AuthenticatedRequest).org.id, parsed.data.status);
  ok(res, null);
}
